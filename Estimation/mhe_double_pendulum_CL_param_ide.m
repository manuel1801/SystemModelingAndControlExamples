...
f = Function('f',{states,controls,process_noise},{DoublePendulumDynamicsNoise(states,controls,process_noise)});
...
W = SX.sym('W',n_process_noise,1);     % process noise over the optimization problem
...
for k = 1:N_MPC
    ...
    k1 = f(X(:,k), U(:,k), W);   
    k2 = f(X(:,k) + dt/2*k1, U(:,k), W); 
    k3 = f(X(:,k) + dt/2*k2, U(:,k), W);
    k4 = f(X(:,k) + dt*k3, U(:,k), W); 
...
% MPC loop
xt = x0;
xt_noisy = xt + 1e-3*(2*rand(n_states,1)-1);
...
u_cl=[];
for k  = 1 : N_sim
    args.p   = [xt_noisy;xs;w]; % Set the parameter vector
    ...
    k1 = f(xt_noisy, u_cl(:,k), w);   
    k2 = f(xt_noisy + dt/2*k1, u_cl(:,k), w); 
    k3 = f(xt_noisy + dt/2*k2, u_cl(:,k), w);
    k4 = f(xt_noisy + dt*k3, u_cl(:,k), w); 
...
% To store estimated state-, and processnoise sequences 
...
W_est_init = (w_max-w_min)/2 * ones(n_process_noise,1); % process noise sequence guess

W = SX.sym('W', n_process_noise,1);   % Estimated process noise sequence
...
for k  = 1 : N_sim
    ...
    if k-1 <= N_MHE % Rebuild optimization problem as long as estimation horizon not reached
        ...
        for i = 1:N_MHE_t
            ...
            obj = obj + W'*Q*W + (y_hat - Y_meas(:,i))'*R*(y_hat - Y_meas(:,i));
            ...
            k1 = f(X(:,i), U(:,i), W);   
            k2 = f(X(:,i) + dt/2*k1, U(:,i), W);
            k3 = f(X(:,i) + dt/2*k2, U(:,i), W);
            k4 = f(X(:,i) + dt*k3, U(:,i), W); 
        ...
        args.lbx(n_states*(N_MHE_t+1)+1:n_states*(N_MHE_t+1)+2) = w_min;
        args.ubx(n_states*(N_MHE_t+1)+1:n_states*(N_MHE_t+1)+2) = w_max;        
    ...
    W_est = sol.x(n_states*(N_MHE_t+1)+1:n_states*(N_MHE_t+1)+2);
   
    % Estimate of current state x_t and process noise w_t
    x_est_cl(:,k) = full(X_est(:,end));

    if norm(x_est_cl(1:2,k)) > 0.2
        w_est_cl(:,k) = full(W_est); 
        W_est_init = W_est;
    else
        w_est_cl(:,k) = full(w_est_cl(:,k-1)); 
        W_est_init = w_est_cl(:,k-1);
    end
    ...
    if mod(k,10)==0
        disp(['Iteration ', num2str(k), ' / ', num2str(N_sim), 'vel = ', num2str(norm(x_est_cl(1:2,k)))])
    end
...
plot(t,x_est_cl([1,2],:)); hold on
plot(t,y_cl([1,2],:)); 
legend({'$\theta_1$','$\theta_2$','$\hat \theta_1$','$\hat \theta_2$', '$y_1$','$y_2$'}, Interpreter="latex")
subplot(2,1,2)
...
figure
subplot(2,1,1)
plot(t,w_est_cl(1,:)); hold on
plot(t,w(1)*ones(1,N_sim), '--');
subplot(2,1,2)
plot(t,w_est_cl(2,:)); hold on
plot(t,w(2)*ones(1,N_sim), '--');
