...
f = Function('f',{states,process_noise},{DoublePendulumDynamics(states,0)+process_noise});
...
x_OL(:,1) = x0;
x_OL_noisy(:,1) = x0 + w_max*(2*rand(n_process_noise,1)-1);
t(1) = 0;
...
for k=1:N_sim
   ...
   k1 = f(x_OL(:,k),         w0);
   k2 = f(x_OL(:,k)+dt/2*k1, w0);
   k3 = f(x_OL(:,k)+dt/2*k2, w0);
   k4 = f(x_OL(:,k)+dt*k3,   w0);
   x_OL(:,k+1) = x_OL(:,k) + full(dt/6*(k1+2*k2+2*k3+k4));
    
   k1 = f(x_OL_noisy(:,k),         w(:,k));
   k2 = f(x_OL_noisy(:,k)+dt/2*k1, w(:,k));
   k3 = f(x_OL_noisy(:,k)+dt/2*k2, w(:,k));
   k4 = f(x_OL_noisy(:,k)+dt*k3,   w(:,k));
   x_OL_noisy(:,k+1) = x_OL_noisy(:,k) + full(dt/6*(k1+2*k2+2*k3+k4));

   y_OL(:,k) = full(h(x_OL_noisy(:,k))) + v(:,k);
...
x_max = ceil(max(max(x_OL))); x_min = -x_max;
y_max = ceil(max(max(x_OL_noisy))); y_min = -y_max;


process_noise = SX.sym('process_noise',n_states,1);     
n_process_noise = length(process_noise);         

x0_hat = [0; 0; 0; 0]; % Initial guess for \hat x(t-N)
...
for k  = 1 : N_sim
    ...
    if k-1 <= N_MHE % Rebuild optimization problem as long as estimation horizon not reached
        ...
        for i = 1:N_MHE_t
            ...
            k1 = f(X(:,i), W(:,i));   
            k2 = f(X(:,i) + dt/2*k1, W(:,i));
            k3 = f(X(:,i) + dt/2*k2, W(:,i));
            k4 = f(X(:,i) + dt*k3, W(:,i)); 
